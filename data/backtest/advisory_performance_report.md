# High-Confidence Advisory Performance Report
**Generated:** 2026-03-27 18:24:25.102292
**History Length:** 450 weeks

## 1. Action Distribution
| Action | Frequency | Count |
|--------|-----------|-------|
| ADD | 1.78% | 8 |
| REDUCE | 0.22% | 1 |
| HOLD | 98.00% | 441 |
| INSUFFICIENT_DATA | 0.00% | 0 |

## 2. Multi-Horizon Precision
| Action | 28d Precision | 84d Precision | 182d Precision |
|--------|---------------|---------------|----------------|
| ADD | 100.0% (N=8) | 100.0% (N=8) | 100.0% (N=8) |
| REDUCE | Inadequate Sample (N=1) | Inadequate Sample (N=1) | Inadequate Sample (N=1) |

## 3. Regime Breakdown
| Regime | Count | Avg Confidence | Conf Std |
|--------|-------|----------------|----------|
| BULLISH_ACCUMULATION | 8 | 80.0 | 0.0 |
| NEUTRAL | 408 | 50.0 | 0.0 |
| OVERHEATED | 34 | 86.5 | 6.5 |

## 3a. Confidence Bucket Precision

### ADD
| Bucket | Count | 28d Precision | 84d Precision | 182d Precision |
|--------|-------|---------------|---------------|----------------|
| High (>80) | 8 | 100.0% (N=8) | 100.0% (N=8) | 100.0% (N=8) |
| Medium (60-79) | 0 | Inadequate Sample (N=0) | Inadequate Sample (N=0) | Inadequate Sample (N=0) |
| Low (<60) | 0 | Inadequate Sample (N=0) | Inadequate Sample (N=0) | Inadequate Sample (N=0) |

### REDUCE
| Bucket | Count | 28d Precision | 84d Precision | 182d Precision |
|--------|-------|---------------|---------------|----------------|
| High (>80) | 0 | Inadequate Sample (N=0) | Inadequate Sample (N=0) | Inadequate Sample (N=0) |
| Medium (60-79) | 1 | Inadequate Sample (N=1) | Inadequate Sample (N=1) | Inadequate Sample (N=1) |
| Low (<60) | 0 | Inadequate Sample (N=0) | Inadequate Sample (N=0) | Inadequate Sample (N=0) |

## 4. False Positive Analysis
| Action | Horizon | FP Count | Sample |
|--------|---------|----------|--------|
| ADD | 28d | 0 | None |
| ADD | 84d | 0 | None |
| ADD | 182d | 0 | None |
| REDUCE | 28d | 0 | None |
| REDUCE | 84d | 0 | None |
| REDUCE | 182d | 0 | None |
